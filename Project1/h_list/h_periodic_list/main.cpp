...
//constants
    ...
    cout << "dt = " << dt << endl;
    int number = 1;
    int number_of_atoms = N_c*N_c*N_c*4;
    ...
    mat r(number_of_atoms,3);     //mult volume by 4, since there are 4 atoms in each fcc cell
    ...
    double length_of_unit_cell = b*N_c;
...
//loop over all boxes
    ...
    {
//loop over all particles in that box
...
//loop over all neighboring (and same) boxes (x,y,z)-directions
...
//loop over all particles in neighboring (and same) boxes
            ...
            {    
                ...
                for (int atom_in_nabo_box = 0; atom_in_nabo_box < fourthList[Nabo_x_value][Nabo_y_value][Nabo_z_value].size(); atom_in_nabo_box++)
                {
                    ...
                    //if (j_temp != i_temp)
                    //{
                    force_calculation(F,r,N_c,b,sigma,i_temp,j_temp); //eq (9). Calculate F_i(t+dt)
                    //}
                }
                for (int k = 0; k < 3; k++) //looping over the three spacial coordinates
                {
